...
project's founding document; status annotations show what shipped. Self-hosted copy —
supersedes the original claude.ai artifact.*
...
- Shared mutable deploy dirs across concurrent build sessions: bit twice; house rule now —
  deploy from a fresh pull of origin/main, sync /opt after.
